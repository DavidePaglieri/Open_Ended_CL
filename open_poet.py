...
def env_reproduce_MC(parent_list, hp, parentPipes):
    child_list = []
    new_env = []
    reward_list = []
    ...
    for k in range(len(parent_list)):
        ...
        if 38<reward:
            child_list.append((new_env[k], deepcopy(parent_list[k][1]), reward))
    return child_list


def compute_novelty_vs_archive(child, archive):
    distances = []
    for point, _, _ in archive:
...
def mutate_envs(EA_list, parentPipes, archive):
    ...
    for m in range(M):
        ...
        if R>38:
            parent_list.append((E,A,R))

    # Reproduce and MC satisfied together
    child_list = env_reproduce_MC(parent_list, hp, parentPipes)
    child_list = rank_by_novelty(child_list, archive)
    admitted = 0
    
    list_of_agents = [a[1] for a in EA_list]
    
    for E, A, _ in child_list:
        A, R = evaluate_agents(E, list_of_agents, hp, parentPipes)
        if 38<R:
            EA_list.append((E,A,R))
            print(f"Admitted with reward of {R}")
            archive.append((E,A,R))
            break
    M = len(EA_list)
    if M>CAPACITY:
        print("Removing oldest")
        num_removals = M-CAPACITY
        remove_oldest(EA_list, num_removals)
    return EA_list


def full_test(policies, hp, parentPipes):
...
if __name__ == "__main__":
    ...
    for i in range(args.iterations):
        ...
        if i>0 and i%N_mutate==0:
            ...
            EAR_list = mutate_envs(EAR_list, parentPipes, archive)

        num_pairs = len(EAR_list)
        ...
        #----- ATTEMPT TRANSFER AND SWTICHING -----#
        if num_pairs>1 and i%N_transfer==0:
            for j in range(num_pairs):
                print(f"Testing all policies on env {j}")
                # evaluate all the policies from all the environment in this current env
                current_env, current_agent, current_best_reward = EAR_list[j]
                all_agents = [EA[1] for EA in EAR_list] # get all agents from the EA_pair_list
                best_agent, best_reward = evaluate_agents(current_env, all_agents, hp, parentPipes)

                if best_reward > current_best_reward:
                    del current_agent
                    EAR_list[j] = (current_env, best_agent, best_reward)
                    print("Transfer successful")
                    
            if ((i+1) % 10) == 0:
                compare.append(full_test(all_agents, hp, parentPipes))
                print(compare[-1])
    ...
    for count, EAR_pair in enumerate(EAR_list):
        encoding_list.append(EAR_pair[0])
        reward_list.append(EAR_pair[2])
