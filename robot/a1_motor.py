"""Motor model for the A1 robot. Implementation taken from the pyBullet
repository (Laikago robot) and adapted where needed to the A1"""
...
class A1MotorModel(object):
    ...
    def set_strength_ratios(self, ratios):
        ...
        Args:
            ratios: The relative strength of motor output. A numpy array ranging 
            from 0.0 to 1.0.
        """
    ...
    def convert_to_torque(self,
        ...
        Args:
            motor_commands: The desired motor angle if the motor is in position
                control mode. The pwm signal if the motor is in torque control
                mode.
            motor_angle: The motor angle observed at the current time step. It 
                is actually the true motor angle observed a few milliseconds ago 
                (pd latency).
            motor_velocity: The motor velocity observed at the current time 
                step, it is actually the true motor velocity a few milliseconds 
                ago (pd latency).
            true_motor_velocity: The true motor velocity. The true velocity is 
                used to compute back EMF voltage and viscous damping.
        ...
        Returns:
            ...
            NB: These two will coincide in the simulation, but might differ in
            the real world A1 robot
        """
